...
<div class="card mb-4">
    <div class="card-header">
        <form method="GET" action="{{ route('admin.reports.detail') }}" class="row g-3">
            <input type="hidden" name="type" value="{{ $type }}">
            <div class="col-md-3">
                <label class="form-label">Từ ngày</label>
                <input type="date" name="date_from" class="form-control" value="{{ request('date_from', $date) }}" required>
            </div>
            <div class="col-md-3">
                <label class="form-label">Đến ngày</label>
                <input type="date" name="date_to" class="form-control" value="{{ request('date_to', $date) }}" required>
            </div>
            <div class="col-md-2">
            ...
            <div class="col-md-4">
                ...
                <div class="btn-group w-100">
                    ...
                    <a href="{{ route('admin.reports.export-csv', ['export_type' => 'detail', 'date_from' => request('date_from', $date), 'date_to' => request('date_to', $date), 'type' => $type]) }}" class="btn btn-success">
    ...
    <div class="card-body">
        <div class="d-flex justify-content-between align-items-center mb-3">
            <div>
                ...
                <h5 class="mb-0">{{ $title }} - Từ {{ \Carbon\Carbon::parse($dateFrom)->format('d/m/Y') }} đến {{ \Carbon\Carbon::parse($dateTo)->format('d/m/Y') }}</h5>
                @else
                <h5 class="mb-0">{{ $title }} - Ngày {{ \Carbon\Carbon::parse($dateFrom ?? $date)->format('d/m/Y') }}</h5>
                @endif
                <small class="text-muted">Tổng: {{ count($orders) }} đơn hàng</small>
            </div>
        </div>
            ...
            <table class="table table-hover">
                <thead>
                    <tr>
                        ...
                        <th>Doanh thu</th>
                        <th>Trạng thái</th>
                        <th>Ngày {{ $type === 'delivered' ? 'giao' : ($type === 'failed' ? 'cập nhật' : 'tạo') }}</th>
                        <th>Ghi chú</th>
                        <th>Thao tác</th>
                    </tr>
                </thead>
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            @if($type === 'delivered' && $order->delivered_at)
                            <small>{{ $order->delivered_at->format('d/m/Y H:i') }}</small>
                            @elseif($type === 'failed' && $order->updated_at)
