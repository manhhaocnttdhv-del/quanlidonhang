                        ...
                        <td>
                            <div class="btn-group" role="group">
                                @php
                                    ...
                                    if ($order->to_warehouse_id) {
                                        $hasBeenReceivedByDestination = \App\Models\WarehouseTransaction::where('order_id', $order->id)
                                    ...
                                    $canAssignDriver = !$order->delivery_driver_id && !$hasBeenReceivedByDestination;
                                @endphp
                                ...
                                @else
                                    @if($order->deliveryDriver)
                                        <span class="badge bg-success me-1" title="Tài xế: {{ $order->deliveryDriver->name }} - {{ $hasBeenReceivedByDestination ? 'Kho đích đã nhận' : 'Đang vận chuyển' }}">
                                            <i class="fas fa-user me-1"></i>{{ $order->deliveryDriver->name }}
                                        </span>
                                        @if($hasBeenReceivedByDestination)
                                            <br><small class="text-muted"><i class="fas fa-check-circle me-1"></i>Kho đích đã nhận</small>
                                        @elseif($order->to_warehouse_id)
                                            <br><small class="text-warning"><i class="fas fa-truck me-1"></i>Đang vận chuyển</small>
                                        @endif
...
<div class="card mb-4" id="receivedFromWarehousesSection">
    <div class="card-header bg-info text-white d-flex justify-content-between align-items-center">
        <div>
            ...
            <small>Danh sách đơn hàng đã được nhận vào kho này từ kho khác - Tổng: {{ count($ordersReceivedFromWarehouses) }} đơn</small>
        </div>
        <div class="d-flex gap-2 align-items-center">
                ...
                <tbody>
                    ...
                    @php
                        ...
                        if ($order->warehouseTransactions && $order->warehouseTransactions->isNotEmpty()) {
                            ...
                            if (stripos($lastInTransaction->notes ?? '', 'Trả đơn hàng') !== false || 
                    ...
                    <tr class="{{ $isReturnOrder ? 'table-warning' : '' }}">
                        ...
                        <td>
                            ...
                            @if($isReturnOrder)
                            <br><small class="text-danger"><i class="fas fa-exclamation-triangle me-1"></i>Trả về kho</small>
                            @endif
                            @else
                            <span class="text-muted">N/A</span>
                            @endif
                        ...
                        <td>
                            ...
                            <br><a href="tel:{{ $order->sender_phone }}" class="btn btn-xs btn-outline-primary btn-sm mt-1" title="Gọi điện">
                        ...
                        <td>
                            ...
                            <strong class="text-warning">{{ number_format($order->return_fee) }} đ</strong>
                            @else
                            <span class="text-muted">Chưa có</span>
                            @endif
                        </td>
                        ...
                        <td>
                            <div class="btn-group" role="group">
                                @if(!$order->delivery_driver_id)
                                <button type="button" class="btn btn-sm btn-success" onclick="assignReturnShipper({{ $order->id }})" title="Gán shipper trả hàng">
                                    <i class="fas fa-user-plus"></i> Gán shipper
                                </button>
                                @elseif($order->status === 'out_for_delivery')
                                <button type="button" class="btn btn-sm btn-success" onclick="updateDeliveryStatus({{ $order->id }}, 'delivered', {{ $order->shipping_fee ?? 0 }}, {{ $order->return_fee ?? 0 }})" title="Trả hàng thành công">
...
<div class="card" id="failedOrdersSection">
    <div class="card-header bg-danger text-white d-flex justify-content-between align-items-center">
        ...
        <div class="d-flex gap-2 align-items-center">
            <form method="GET" action="{{ route('admin.delivery.index') }}" id="filterProvinceFailedForm" class="mb-0 d-inline-block">
                @if(request('province_delivery'))
                <input type="hidden" name="province_delivery" value="{{ request('province_delivery') }}">
                @endif
                @if(request('province_in_transit'))
                <input type="hidden" name="province_in_transit" value="{{ request('province_in_transit') }}">
                @endif
                <select name="province_failed" id="filterProvinceFailed" class="form-select form-select-sm" onchange="this.form.submit()">
                    <option value="">Tất cả tỉnh/TP</option>
                    @foreach($provinces ?? [] as $province)
                    <option value="{{ $province }}" {{ request('province_failed') == $province ? 'selected' : '' }}>{{ $province }}</option>
                    @endforeach
                </select>
            </form>
        </div>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <small class="text-muted" title="{{ $order->failure_reason }}">{{ Str::limit($order->failure_reason, 50) }}</small>
                            @else
                        ...
                        <td>
                            <div class="btn-group" role="group">
                                <button type="button" class="btn btn-sm btn-warning" onclick="returnToWarehouse({{ $order->id }})" title="Quay giao hàng để trả về kho">
                ...
                <div class="modal-body">
                    ...
                    <div id="deliveredForm" style="display: none;">
                        <div class="mb-3" id="shippingFeeField">
                            <label class="form-label">Phí vận chuyển đã thu (đ) <span class="text-danger">*</span></label>
                            <input type="number" name="shipping_fee" class="form-control" id="shippingFeeInput" min="0" step="0.01" value="30000">
                            <small class="text-muted">Nhập số tiền phí vận chuyển đã thu từ khách hàng (mặc định: 30,000 đ)</small>
                        </div>
                        <div class="mb-3" id="returnFeeField" style="display: none;">
                            <label class="form-label">Phí trả hàng đã thu (đ) <span class="text-danger">*</span></label>
                            <input type="number" name="return_fee_collected" class="form-control" id="returnFeeInput" min="0" step="0.01">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea name="delivery_notes" class="form-control" rows="3" placeholder="Ghi chú về việc giao hàng (nếu có)"></textarea>
                        </div>
                    </div>
                    ...
                    <div id="failedForm" style="display: none;">
                        <div class="mb-3">
                            ...
                            <textarea name="failure_reason" class="form-control" rows="3" id="failureReasonInput" placeholder="Ví dụ: Khách hàng không nhận, địa chỉ sai, không liên lạc được..."></textarea>
                            <small class="text-danger" id="failureReasonError" style="display: none;">Vui lòng nhập lý do giao hàng thất bại</small>
                        </div>
                        <div class="mb-3">
                        ...
                        <div class="mb-3">
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" name="return_to_warehouse" id="returnToWarehouse" value="1" checked>
                                <label class="form-check-label" for="returnToWarehouse">
                                    Trả đơn hàng về kho để phân công lại
                                </label>
                            </div>
                            <small class="text-muted">Nếu chọn, đơn hàng sẽ được trả về kho và có thể phân công lại shipper khác</small>
                        </div>
                    </div>
                </div>
                <div class="modal-footer">
            ...
            <form id="assignReturnShipperForm" method="POST">
                ...
                <div class="modal-body">
                    <div class="alert alert-warning">
                        <i class="fas fa-exclamation-triangle me-2"></i>
                        <strong>Lưu ý:</strong> Đây là đơn hàng trả về. Shipper sẽ trả hàng cho người gửi hoặc đưa về kho.
                    </div>
                    <div class="mb-3">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" name="return_fee" class="form-control" min="0" step="0.01" value="30000" placeholder="Nhập phí trả hàng">
                        <small class="text-muted">Phí trả hàng mặc định: 30,000 đ. Sẽ được thu từ người gửi khi shipper trả hàng</small>
                    </div>
                    <div class="mb-3">
            ...
            <form id="bulkAssignDriverForm" method="POST" action="{{ route('admin.delivery.bulk-assign-driver') }}">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        <small class="text-muted" id="bulkDriverSelectHint">Chọn tài xế phù hợp với loại đơn hàng đã chọn</small>
                    </div>
                    <div class="mb-3">
                        <label class="form-label">Thời gian giao dự kiến</label>
                        <input type="datetime-local" name="delivery_scheduled_at" class="form-control" value="{{ date('Y-m-d\TH:i') }}">
...
function assignDriver(orderId, toWarehouseId) {
    ...
    const hasToWarehouse = toWarehouseId && toWarehouseId !== null && toWarehouseId !== 'null' && toWarehouseId !== '' && !isNaN(toWarehouseIdNum) && toWarehouseIdNum > 0;
    ...
    loadShippersToSelect('#driverSelect', driverType, function() {
        if (hasToWarehouse) {
            ...
            $('#driverSelectHint').html('<strong>Lưu ý:</strong> Đơn hàng đã xuất từ kho cần chuyển đến kho khác. Chỉ chọn <strong>tài xế vận chuyển tỉnh</strong>.');
...
function bulkAssignDriver(section) {
    ...
    loadShippersToSelect('#bulkDriverSelect', driverType, function() {
        if (section === 'shippedOut') {
            $('#bulkDriverSelectHint').html('<strong>Lưu ý:</strong> Đơn hàng đã xuất từ kho cần chuyển đến kho khác. Chỉ chọn <strong>tài xế vận chuyển tỉnh</strong>.');
            ...
            if (response.success && response.drivers) {
                ...
                response.drivers.forEach(function(driver) {
                    const label = driver.name + ' - ' + driver.phone + 
                        (driver.driver_type === 'shipper' ? ' (Shipper)' : ' (Vận chuyển tỉnh)');
...
function assignReturnShipper(orderId) {
    $('#assignReturnShipperForm').attr('action', '{{ route("admin.delivery.assign-return-shipper", ":id") }}'.replace(':id', orderId));
...
function assignDeliveryDriver(orderId, toWarehouseId) {
    $('#orderId').val(orderId);
    $('#assignDriverForm').attr('action', '{{ route("admin.delivery.assign-driver", ":id") }}'.replace(':id', orderId));
...
function updateDeliveryStatus(orderId, status, shippingFee = 0, returnFee = 0) {
    ...
    // Set form action
    $('#updateDeliveryStatusForm').attr('action', '{{ route("admin.delivery.update-status", ":id") }}'.replace(':id', orderId));
    ...
    if (status === 'delivered') {
        ...
        if (isReturnOrder) {
            ...
            $('#returnFeeInput').val(returnFee && returnFee > 0 ? returnFee : 30000);
            ...
            // Phí vận chuyển mặc định là 30,000 đ
            $('#shippingFeeInput').val(30000);
...
$(document).on('submit', '#updateDeliveryStatusForm', function(e) {
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            if (typeof showNotification === 'function') {
                showNotification('success', response.message || 'Đã cập nhật trạng thái giao hàng thành công!');
            } else {
...
function returnToWarehouse(orderId) {
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.success) {
                if (typeof showNotification === 'function') {
                    showNotification('success', response.message || 'Đã trả đơn hàng về kho gửi ban đầu thành công.');
                } else {
