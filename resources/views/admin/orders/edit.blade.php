...
<div class="card">
    <div class="card-header">
        <h5 class="mb-0"><i class="fas fa-edit me-2"></i>Sửa đơn hàng: {{ $order->tracking_number }}</h5>
    </div>
    <div class="card-body">
            ...
            <div class="row">
                <div class="col-md-6">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="sender_name" id="sender_name" class="form-control" required value="{{ old('sender_name', $order->sender_name) }}">
                    </div>
                    <div class="mb-3">
                        <label class="form-label">Số điện thoại <span class="text-danger">*</span></label>
                        <input type="text" name="sender_phone" id="sender_phone" class="form-control" required value="{{ old('sender_phone', $order->sender_phone) }}">
                    </div>
                    <div class="mb-3">
                        <label class="form-label">Địa chỉ <span class="text-danger">*</span></label>
                        <textarea name="sender_address" id="sender_address" class="form-control" rows="2" required>{{ old('sender_address', $order->sender_address) }}</textarea>
                    </div>
                    <div class="row">
                        <div class="col-md-12">
                            <label class="form-label">Kho gửi</label>
                            @php
                                $warehouse = $order->warehouse ?? \App\Models\Warehouse::getDefaultWarehouse();
                            @endphp
                            ...
                            <input type="hidden" name="sender_province" id="sender_province" value="{{ old('sender_province', $warehouse->province ?? 'Nghệ An') }}">
                            <input type="hidden" name="sender_district" id="sender_district" value="{{ old('sender_district', $warehouse->district ?? '') }}">
                            <input type="hidden" name="sender_ward" id="sender_ward" value="{{ old('sender_ward', $warehouse->ward ?? '') }}">
                        </div>
                    </div>
                    <div class="mb-3">
                        <label class="form-label">Phương thức nhận hàng <span class="text-danger">*</span></label>
                        <div class="form-check">
                            <input class="form-check-input" type="radio" name="pickup_method" id="pickup_method_driver" value="driver" {{ old('pickup_method', 'driver') == 'driver' ? 'checked' : '' }}>
                        ...
                        <div class="form-check mt-2">
                            <input class="form-check-input" type="radio" name="pickup_method" id="pickup_method_warehouse" value="warehouse" {{ old('pickup_method', 'driver') == 'warehouse' ? 'checked' : '' }}>
                            <label class="form-check-label" for="pickup_method_warehouse">
                                <i class="fas fa-warehouse me-1"></i>Đưa đến kho
                            </label>
                            <small class="d-block text-muted ms-4">Người gửi tự đưa hàng đến kho, đơn hàng sẽ tự động có mặt tại kho</small>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-md-6">
                    ...
                    <input type="hidden" name="receiver_district" id="receiver_district" value="{{ old('receiver_district', $order->receiver_district ?? '') }}">
                    <div class="mb-3">
                        <label class="form-label">Địa chỉ chi tiết (Số nhà, tên đường) <span class="text-danger">*</span></label>
                        <textarea name="receiver_address" id="receiver_address" class="form-control" rows="2" required placeholder="Nhập địa chỉ chi tiết sau khi đã chọn Tỉnh/Thành và Phường/Xã" disabled>{{ old('receiver_address', $order->receiver_address) }}</textarea>
            ...
            <div class="row">
                <div class="col-md-6">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" name="weight" class="form-control" step="0.01" min="0" required value="{{ old('weight', $order->weight) }}" id="weight">
                    </div>
                    <div class="row">
                        <div class="col-md-4">
                            <label class="form-label">Chiều dài (cm)</label>
                            <input type="number" name="length" class="form-control" step="0.01" min="0" value="{{ old('length', $order->length) }}">
                        </div>
                        <div class="col-md-4">
                            <label class="form-label">Chiều rộng (cm)</label>
                            <input type="number" name="width" class="form-control" step="0.01" min="0" value="{{ old('width', $order->width) }}">
                        </div>
                        <div class="col-md-4">
                            <label class="form-label">Chiều cao (cm)</label>
                            <input type="number" name="height" class="form-control" step="0.01" min="0" value="{{ old('height', $order->height) }}">
                        </div>
                    </div>
                    <div class="mb-3 mt-3">
                        <div class="form-check">
                            <input class="form-check-input" type="checkbox" name="is_fragile" value="1" id="is_fragile" {{ old('is_fragile', $order->is_fragile) ? 'checked' : '' }}>
                ...
                <div class="col-md-6">
                    ...
                    <div class="mb-3">
                        ...
                        <select name="service_type" class="form-select" id="service_type">
                            <option value="standard" {{ old('service_type', $order->service_type) == 'standard' ? 'selected' : '' }}>Tiêu chuẩn</option>
                            <option value="express" {{ old('service_type', $order->service_type) == 'express' ? 'selected' : '' }}>Hỏa tốc</option>
                            <option value="economy" {{ old('service_type', $order->service_type) == 'economy' ? 'selected' : '' }}>Tiết kiệm</option>
                        </select>
                    </div>
                    <div class="mb-3">
                        <label class="form-label">Tiền thu hộ COD (đ)</label>
                        <input type="number" name="cod_amount" class="form-control" step="0.01" min="0" value="{{ old('cod_amount', $order->cod_amount ?? 0) }}" id="cod_amount">
                    </div>
                    <div class="mb-3">
...
$(document).ready(function() {
    ...
    function initializeAddressSelects() {
        ...
        @if($order->receiver_province)
            receiverProvinceSelect.val('{{ $order->receiver_province }}').trigger('change');
        @endif
    }
    ...
    function loadWardsForProvince(provinceName) {
        ...
        if (province && province.districts) {
            ...
            @if($order->receiver_ward)
                $wardSelect.val('{{ $order->receiver_ward }}').trigger('change');
            @endif
        } else {
            ...
            success: function(warehouses) {
                ...
                if (warehouses.length === 0) {
                    ...
                    @if($order->to_warehouse_id)
                        $warehouseSelect.val({{ $order->to_warehouse_id }});
                    @endif
                }
            },
            error: function(xhr) {
    ...
    // Initialize customer data if editing
    ...
    // Initial calculation
    @if($order->receiver_province && $order->weight)
        setTimeout(function() {
            calculateShippingFee();
        }, 500);
    @endif
